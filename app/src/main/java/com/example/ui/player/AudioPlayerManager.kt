    ...
    fun play(song: Song) {
        ...
        try {
            mediaPlayer = MediaPlayer().apply {
                ...
                val localFile = song.localFilePath?.let { java.io.File(it) }
                if (song.isDownloaded && localFile != null && localFile.exists()) {
                    setDataSource(localFile.absolutePath)
                    Log.d("AudioPlayerManager", "Playing from offline cached source: ${localFile.absolutePath}")
                } else if (urlToPlay.startsWith("http://") || urlToPlay.startsWith("https://")) {
